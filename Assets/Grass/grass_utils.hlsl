...
float2 bezierTangent(float2 p, float2 l, float t)
{
    return 2 * p * (1 - t) + 2 * t * (l - p);
}

float2 bezierNormal(float2 p, float2 l, float t)
{
    float2 tangent =  2 * p * (1 - t) + 2 * t * (l - p);
    return float2(tangent.y, -tangent.x);
}

float3 computeBezierPos(float2 uv, float height, float halfWidth, float2 midPoint, float2 tipPoint)
{
